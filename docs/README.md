...
`돈을 충분히 넣고` 라... 너무 애매한 표현인걸?
이미 전 단계 기능을 구현할 때 자판기에 돈이 남아있으면 음료 구매가 계속해서 가능하게 만들었었는데... 지금 보면 이미 요구사항 6을 충족 시켰버렸단 말이지

혹시.. 😱 전 단계까지는 음료버튼을 누르면 바로 잔돈이 반환되는 거였나?
객체와의 소통인데 너와 제대로 소통하지 못했네

그러고 보니 지금까지 공개된 요구사항들을 생각해 보면, 너는 최소한 기능단위로 역할을 축소해서 구현을 해왔어야 하지 싶다

하지만 이번에는 이미 구현되버려 버린 이상 이번 단계는 리팩토링에 중점을 두겠어!! 다 때려 고쳐주마😎
...
`환불레버가 있다` 흠... 🤔 동전함으로 잔돈이 나온다는 거랑 어떻게 다르게 구현을 해주어야 할지 모르겠어!
답을 줘 자판기!
이전 단계의 잔돈이 나온다랑 어떻게 다르게 구현을 해줘야 하지?

이전단계에서 이미 음료를 구매하지 않아도 잔돈반환버튼을 누르면 자판기가 가지고 있는 돈이 모두 반환이 되도록 구현을 해 놓았단 말이지...🤔

일단은 요구사항을 구현해야 하기 때문에 환불레버를 누르면, 잔돈 반환버튼을 눌렀을 때 처럼 동일하게 작동하도록 구현할께!

## 객체와 합의 본 사항 📝

- [x] 환불 버튼 요소를 추가한다
- [x] 환불 버튼을 누르면, 현재 투입된 금액이 모두 반환된다
- [x] 동전함과 동일한 곳에 금액이 표시된다
...
이번에는 돈을 돌려주는 기능이 추가되었네 그래 돈이 남으면 돌려줘야지!
그러면 언제 돈을 돌려줘야할까? 구매하기 버튼을 눌렀을 때에 바로 반환해 줄 수 도 있고, 반환하기 버튼을 따로 만들어서 작동시킬 수도 있는데 말이야. 역시 반환하기 버튼을 눌렀을때에 돌려주는게 유저의 사용성 측면에서 더 낫겠지?

추가로 반환하기 버튼이 상품 구매하는 화면에 있어야 할 것 같아.

이번 단계부터는 CoffeeMachineComponent를 여러개의 component로 분리해볼까?
모든 기능이 해당 component에 구현하니깐 CoffeeMachineComponent의 가독성이 떨어지는 것 같아.

현재 큰 기능으로는 재료 충전, 상품구매하기, 잔돈 충전하기 등이 있는데 말이야 흠... 🤔

일단은 재료 충전 부분만 component 분리를 해서 진행해보자 더 나누는건 더 생각해봐야할 것 같아

## 객체와 합의 본 사항 📝

- [x] 반환버튼은 상품구매 화면에 위치한다.
- [x] 반환버튼을 누르면 돈이 반환된다
- [x] 반환되는 금액이 동전함으로 나온다

## 이전단계와 달라지는 점들

- [x] 재료 충전하기 component를 분리한다
- [x] 각 메뉴들의 금액이 표기 된다
- [x] 메뉴가 나올때에 재료가 순서대로 나온다.
...
- [x] 투입된 금액을 유저에게 알린다.
- [x] 투입된 금액이하의 가격을 가진 메뉴들을 버튼들이 활성화 된다.
- [x] 메뉴들의 가격은 아래와 같다.
  - [x] 에스프레소 1000원
  - [x] 아메리카노 1500원
  - [x] 카페라떼 2000원
  - [x] 우유 1500원
- [x] 활성화 된 메뉴버튼을 클릭할 경우, 해당 메뉴의 수량과 메뉴의 가격만큼 로컬스토리지가 업데이트된다

### 기존과 바뀌는 점들

- [x] 음료들을 drink 객체에서 관리
